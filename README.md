...
当前版本 **0.24.0**：同日同栏目合为一页，保留多段正文；移除印象角度选项；API 设置移到酒馆扩展面板；增加五种寄信动画，并缩小装饰图片。无需清空旧数据。

一个运行于 SillyTavern 的纯前端 UI 扩展。它借助当前已配置的模型，把当前聊天、角色卡、User Persona 与本次生成实际激活的世界书内容，整理成一本按聊天独立保存的私人日志。

本次变更与更新方法见 [0.24.0 更新说明](更新说明-0.24.0.md)。
...
- 在 SillyTavern“扩展程序 → 私语手札 → API 设置”中切换“跟随正文 API”或“副 API”；先选择 API 名称，再选择酒馆中已保存的连接配置，最后拉取模型列表或手动输入模型 ID。设置不再放在日记本内，手机端面板随扩展抽屉宽度排版。
...
运行版本会显示在手札内页底部，例如 `v0.24.0`；浏览器控制台也会打印 `[Private Journal v0.24.0] initialize:ready` 以及 SillyTavern 版本、站点 origin、IndexedDB/LocalForage 状态和当前扩展抽屉容器。`manifest.json` 中的 JS/CSS 路径也带有 `v=0.24.0` 缓存戳，避免手机继续读取旧版资源。
...
```js
window.__stPrivateJournalRuntime.version        // 必须是 0.24.0，否则是缓存/安装问题
