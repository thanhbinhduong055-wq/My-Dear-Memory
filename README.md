...
当前修正版的使用入口、安装方式、截断补全次数和验证边界，见 [0.23.1 更新说明](更新说明-0.23.1.md)。
...
运行版本会显示在手札内页底部，例如 `v0.23.1`；浏览器控制台也会打印 `[Private Journal v0.23.1] initialize:ready` 以及 SillyTavern 版本、站点 origin、IndexedDB/LocalForage 状态和当前扩展抽屉容器。`manifest.json` 中的 JS/CSS 路径也带有 `v=0.23.1` 缓存戳，避免手机继续读取旧版资源。
...
```js
window.__stPrivateJournalRuntime.version        // 必须是 0.23.1，否则是缓存/安装问题
