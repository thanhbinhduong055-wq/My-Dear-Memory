...
运行版本会显示在手札内页底部，例如 `v0.22.0`；浏览器控制台也会打印 `[Private Journal v0.22.0] initialize:ready` 以及 SillyTavern 版本、站点 origin、IndexedDB/LocalForage 状态和当前扩展抽屉容器。`manifest.json` 中的 JS/CSS 路径也带有 `v=0.22.0` 缓存戳，避免手机继续读取旧版资源。
...
```js
window.__stPrivateJournalRuntime.version        // 必须是 0.22.0，否则是缓存/安装问题
...
- 普通批量输出预算提高为 10000 tokens，跨阶段多篇输出为 24000 tokens，给最后的恋爱日记保留空间；这仍是一次请求，上限不等于实际用量。服务端或模型自身的输出限制仍可能使响应提前结束；未闭合正文会标注“响应未写完”，保留收到的文字，供手动编辑或重新生成。旧数据中从未收到的结尾无法自动恢复。
