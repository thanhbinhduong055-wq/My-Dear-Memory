...
运行版本会显示在手札内页底部，例如 `v0.21.3`；浏览器控制台也会打印 `[Private Journal v0.21.3] initialize:ready` 以及 SillyTavern 版本、站点 origin、IndexedDB/LocalForage 状态和当前扩展抽屉容器。`manifest.json` 中的 JS/CSS 路径也带有 `v=0.21.3` 缓存戳，避免手机继续读取旧版资源。
...
```js
window.__stPrivateJournalRuntime.version        // 必须是 0.21.3，否则是缓存/安装问题
