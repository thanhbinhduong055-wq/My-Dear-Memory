...
with sync_playwright() as p:
  ...
  check('runtime.version === 0.22.0', rt['v']=='0.22.0', rt)
  check('documentElement dataset marker', rt['html']=='0.22.0', rt)
  check('root dataset marker', rt['root']=='0.22.0', rt)
  ...
  check('stylesheet ok + marker matches', ss['status']=='ok' and ss['marker']=='0.22.0', ss)
  c.close()
  ...
  print('\n=== 8b. 过期 CSS（JS 0.22.0 + CSS 0.21.2 缓存）===')
  c=ctx(b); pg=c.new_page()
  def stale(route):
    body=open(__import__('pathlib').Path(__file__).resolve().parents[2]/'style.css',encoding='utf-8').read().replace('--pj-stylesheet-version:"0.22.0"','--pj-stylesheet-version:"0.21.2"')
