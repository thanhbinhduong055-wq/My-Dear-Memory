...
(async()=>{
  ...
  try {
    for(const width of [390,1280]){
      ...
      await page.evaluate(()=>taskTest.c.extensionSettings.st_private_journal.followMainGeneration=false);
      await page.locator('#private-journal-launcher').click();
      await page.locator('.pj-cover').click();
      await page.locator('[data-action="choose-period"]').click();
      await page.locator('[data-batch-from]').fill('2025-09-24');
      await page.locator('[data-batch-to]').fill('2025-09-25');
      const panel=page.locator('.pj-period-panel');
      const rect=await panel.boundingBox();
      assert.ok(rect.x>=0 && rect.x+rect.width<=width+1 && rect.y>=0,'period panel fits viewport');
      fs.mkdirSync(path.join(candidate,'..','playwright','0.25.3'),{recursive:true});
      await page.screenshot({path:path.join(candidate,'..','playwright','0.25.3',`period-${width}.png`)});
      await page.locator('[data-action="generate-period"]').click();
      await page.waitForFunction(()=>taskTest.calls.length===11);
      assert.equal(await page.locator('[data-action="generate-period"]').isDisabled(),true);
      const prompt=await page.evaluate(()=>JSON.stringify(taskTest.calls[10].prompt));
      assert.match(prompt,/selected_story_period/);
      assert.match(prompt,/2025-09-24 至 2025-09-25/);
      await page.evaluate(()=>taskTest.calls[10].resolve(JSON.stringify({relationship:{status:'partners',reason:'已确认恋爱',evidence:[]},updates:['impression','daily_note','love_letter','romance_diary'].map(type=>({type,page:{title:'手动整理',body:'所选时间段的完整回忆。',dateLabel:'2025-09-24 至 2025-09-25'}}))})));
      await page.waitForFunction(()=>Object.keys(taskTest.c.chatMetadata.st_private_journal.manualPeriodCaptures||{}).length===1);
      assert.equal(await page.evaluate(()=>taskTest.c.chatMetadata.st_private_journal.pages.filter(p=>p.source==='manual-batch').length),4);
      await page.locator('[data-action="generate-period"]').click();
      await page.waitForTimeout(150);
      assert.equal(await page.evaluate(()=>taskTest.calls.length),11,'same range/source is deduplicated');
      await page.locator('[data-batch-from]').fill('2025-01-01');
      await page.locator('[data-batch-to]').fill('2025-01-02');
      await page.locator('[data-action="generate-period"]').click();
      assert.match(await page.locator('[data-batch-feedback]').textContent(),/没有找到/);
      assert.equal(await page.evaluate(()=>taskTest.calls.length),11,'empty range costs no API');
      await page.evaluate(()=>taskTest.c.extensionSettings.st_private_journal.generationApiMode='main');
      await page.locator('[data-batch-from]').fill('2025-09-26');
      await page.locator('[data-batch-to]').fill('2025-09-26');
      await page.locator('[data-action="generate-period"]').click();
      await page.waitForFunction(()=>taskTest.calls.length===12);
      assert.equal(await page.evaluate(()=>taskTest.calls[11].mode),'main');
      await page.evaluate(()=>taskTest.calls[11].resolve(JSON.stringify({relationship:{status:'partners'},updates:['impression','daily_note','love_letter','romance_diary'].map(type=>({type,page:{title:'单日回忆',body:'当天的完整回忆。',dateLabel:'2025-09-26'}}))})));
      await page.waitForFunction(()=>Object.keys(taskTest.c.chatMetadata.st_private_journal.manualPeriodCaptures||{}).length===2);
      await page.locator('[data-batch-from]').fill('2025-09-25');
      await page.locator('[data-batch-to]').fill('2025-09-25');
      await page.locator('[data-action="generate-period"]').click();
      await page.waitForFunction(()=>taskTest.calls.length===13);
      await page.evaluate(()=>taskTest.calls[12].resolve('<journal_batch><page type="impression" save="true"><title>中断</title><body>收到的部分文字。'));
      await page.waitForFunction(()=>!document.querySelector('[data-action="generate-period"]').disabled);
      assert.equal(await page.evaluate(()=>taskTest.calls.length),13,'truncated manual result never adds paid continuation');
      assert.equal(await page.evaluate(()=>Object.keys(taskTest.c.chatMetadata.st_private_journal.manualPeriodCaptures||{}).length),2,'incomplete request remains retryable');
